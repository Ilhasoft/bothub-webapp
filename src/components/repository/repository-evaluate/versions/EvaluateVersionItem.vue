  ...
  <div
    ...
    <div
      ...
      <div class="version-test-item__wrapper__check">
        <b-checkbox v-model="evaluateSelected" :disabled="checkAmountOfEvaluates"/>
      </div>
      <div class="version-test-item__wrapper__info">
        <evaluate-type-tag :evaluate-type="cross_validation"/>
        <language-badge
          :language="language"
          class="version-test-item__wrapper__info__item"
          main/>
        <span class="version-test-item__wrapper__info__item">
          {{ $t('webapp.result.test') }} #{{ id }}
        </span>
...
export default {
  ...
  watch: {
    evaluateSelected(){
      this.setSelectedEvaluate(this.id)
    }
  },
  computed: {
    ...mapGetters([
    ...
    checkCrossValidation(){
      if (this.cross_validation) return 'cross-validation'
      return 'evaluate'
    },
    checkAmountOfEvaluates(){
      return this.getEvaluatesToCompare.length === 2
      && !this.getEvaluatesToCompare.includes(this.id)
    },
    resultRouterParams() {
      return {
        ...
        params: {
          resultId: this.id,
          crossValidation: this.checkCrossValidation
        },
      };
    },
  },
